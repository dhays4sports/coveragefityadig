...
(async () => {
  ...
  check('Workspace exposes visible consultation-document action', workspaceHtml.includes('id="openConsultationDocument"') && (workspaceHtml.includes('Open consultation document') || workspaceHtml.includes('Agent Guide')));
  ...
  check('document route contains a titled preview frame', consultationHtml.includes('id="consultationDocumentFrame"') && consultationHtml.includes('title="CoverageFit call-ready consultation guide preview"'));
  ...
  check('document controller exposes stable public API', documentApi && ['1.0.0','1.1.0','1.2.0'].includes(documentApi.VERSION) && typeof documentApi.renderDocument === 'function');
  ...
  check('generated output identifies Agent Guide', rendered.output.html.includes('Agent Guide') && rendered.output.html.includes('Confidential agent consultation document'));
