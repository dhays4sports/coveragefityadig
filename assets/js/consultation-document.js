(function (root, factory) {
  ...
  function buildRenderOptions(record, snapshot, plan, checklistState, producer, preferences) {
    ...
    return {
      ...
      consultationDate,
      title: 'Home Coverage Consultation Guide',
      preparedBy: producerData.preparedBy,
      agency: producerData.agency,
      rendererOptions: {
        title: `${customerName} | Agent Guide | CoverageFit`,
        reportShellOptions: {
          title: 'Home Coverage Consultation Guide',
          documentLabel: 'Agent Guide',
  ...
  function present(result) {
    ...
    const completePreview = () => {
      ...
      previewReady = true;
      setState('ready');
      announce(`Call-ready consultation guide ready for ${text(result.snapshot.customer?.name, 'the selected homeowner')}.`);
    };
    ...
    root.document.title = `${text(result.snapshot.customer?.name, 'Homeowner')} | Agent Guide | CoverageFit`;
  }
  ...
  function fail(error) {
    ...
    announce('Consultation guide unavailable.');
  }
