(function (root, factory) {
  ...
  const section = Object.freeze({
    ...
    render(model) {
      ...
      const coverageFacts = [
        ['Replacement cost', currency(m.coverage.replacementCost), 'Recorded rebuilding or dwelling amount'],
        ['Deductible', currency(m.coverage.deductible), 'Recorded property deductible'],
        ['Current carrier', present(m.coverage.currentCarrier) ? display(m.coverage.currentCarrier) : '', 'Carrier reported during the review'],
        ['Current annual premium', currency(m.coverage.currentPremium), 'Premium reported during the review'],
        ['Renewal / expiration', date(m.coverage.renewalDate), 'Date reported during the review']
      ].filter(([,value]) => present(value));
      ...
      const overviewFacts = [
        ...
        ['Carrier', present(m.coverage.currentCarrier) ? display(m.coverage.currentCarrier) : '']
      ].filter(([,value]) => present(value));
      ...
      if (constructionFacts.length) {
        sectionNumber += 1;
        panels.push(`<section class="cf-property-panel" aria-labelledby="cf-property-details-heading">
          ${panelHeading(sectionNumber, 'Home details', 'Known property information', 'cf-property-details-heading')}
      ...
      if (coverageFacts.length) {
        sectionNumber += 1;
        panels.push(`<section class="cf-property-panel cf-property-coverage-panel" aria-labelledby="cf-property-coverage-heading">
          ${panelHeading(sectionNumber, 'Policy details', 'Recorded information to check', 'cf-property-coverage-heading')}
          ...
          <p class="cf-property-coverage-note">Confirm every policy amount, term, and date against the current declarations page and issued policy before making a recommendation.</p>
        </section>`);
      }

      if (risks.length) {
        sectionNumber += 1;
        panels.push(`<section class="cf-property-panel cf-property-risks" aria-labelledby="cf-property-risks-heading">
          ${panelHeading(sectionNumber, 'Discuss', 'Property items worth reviewing', 'cf-property-risks-heading')}
      ...
      const detailsMarkup = panels.length
        ...
        : `<section class="cf-property-empty-state" aria-label="Property details not provided">
            ...
            <p>Use the consultation to confirm construction details, current policy information, and any property features that may affect the coverage discussion.</p>
          </section>`;
  ...
  <header class="cf-property-header">
    <div class="cf-property-heading-copy">
      <p class="cf-property-eyebrow">Property and policy</p>
      <h1 id="cf-property-title">Home and Policy Details</h1>
      <p class="cf-property-address">${address ? display(address) : 'Property address not provided'}</p>
    </div>
    <div class="cf-property-brand" aria-label="CoverageFit">CoverageFit<span>®</span><small>Agent Guide</small></div>
  </header>

  ${overviewFacts.length ? `<section class="cf-property-overview" aria-label="Known property overview">${overviewFacts.map(([label,value]) => overview(label,value)).join('')}</section>` : ''}

  ${detailsMarkup}

  <footer class="cf-property-footer">
    <p><strong>Purpose:</strong> Use only the information actually provided, then confirm missing or policy-specific details before recommendations are finalized.</p>
    <p>CoverageFit · Confidential agent material</p>
    <strong class="cf-document-section">Home details</strong>
  </footer>
</section>`;
